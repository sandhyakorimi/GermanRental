...
export default function TenantDashboard() {
  ...
  const myInquiries = useMemo(
    () =>
      inquiries.filter(
        (i) =>
          i.tenantEmail === user?.email ||
          i.tenantName === user?.name,
      ),
    [inquiries, user],
  )

  const recommended = useMemo(() => {
    const favCities = savedHomes.map(
      (p) => p.city,
    )
        ...
        empty={
          myInquiries.length === 0 ? (
            <EmptyState
              ...
              description="When you contact a landlord, your messages will appear here."
              action={
                <Link
                  to="/properties"
                  className="btn-primary"
